...
export default function AboutMe() {
  ...
  const handleSubmit = () => {
    ...
    const payload = {
      ...
      endorsements: formData.endorsements,
      invitation: formData.invitation,
      peer_project_email: formData.peer_project_email  // 👈 INCLUDED IN PAYLOAD
    };
      ...
      .withSuccessHandler((result) => {
        if (result.success || result.status === 'success') {
          ...
          setFormData({
            ...
            endorsements: [],
            invitation: [],
            peer_project_email: ''  // 👈 RESET ON SUCCESS
          });
          console.log('✅ Success:', result);
        } else {
          // API ran but returned an error status
        ...
        {isExpanded && (
          <div className="p-4 border-t border-gray-100">
            <div className="space-y-4">
              ...
              {/* Join a peers project – stored in formData.peer_project_email */}
              <div className="">
                <label className="block text-sm font-medium text-gray-700 mb-2">
                ...
                <div className="flex">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        ...
                        // Normalize to trimmed value in formData
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      // Normalize to trimmed value in formData
                      handleInputChange('peer_project_email', trimmed);
                      // If you later add a join action, trigger it here.
                    }}
                    disabled={isSubmitting}
                    className="px-3 py-2 bg-indigo-600 text-white rounded-r-md hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 disabled:opacity-50 transition"
                    aria-label="Confirm peer email"
                    title="Confirm peer email"
                  >
                    <Plus className="w-4 h-4" />
                  </button>
                </div>
