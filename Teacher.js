...
function getGateStandards(projectId, invokerEmail) {
  ...
  const payload = {
    ...
    payload: {
      invoker_email: String(invokerEmail),
      project_id: String(projectId),
    },
  };
  ...
  // Parse body (API returns a stringified body)
  const body =
    out && typeof out.body === "string"
      ? JSON.parse(out.body)
      : out.body;

  // Extract standards (handle flexible API shapes)
  let standards =
    (body && body.standards) ||
    (body && body.data && body.data.standards) ||
    (body && body.gate_standards) ||
    (Array.isArray(body) ? body : null);

  // If still not found, deep-scan
  if (!standards) {
    (function findStandards(o) {
      if (!o || typeof o !== "object" || standards) return;

      if (o.standards || o.gate_standards) {
        standards = o.standards || o.gate_standards;
        return;
      }

      for (const k in o) {
        const v = o[k];
        if (Array.isArray(v)) v.forEach(findStandards);
        else findStandards(v);
        if (standards) return;
      }
    })(body || out);
  }
  ...
  return {
    statusCode: out.statusCode || 200,
    body: { standards: standards || null, debug: !standards ? body : undefined },
  };
}
